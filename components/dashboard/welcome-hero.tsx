...
export function WelcomeHero() {
  ...
  return (
    <div className="glass-card p-5 sm:p-8 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-48 h-48 sm:w-64 sm:h-64 bg-amber-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
      <div className="absolute bottom-0 left-0 w-32 h-32 sm:w-48 sm:h-48 bg-emerald-500/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4" />
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
        ...
        <div className="glass-panel p-3 sm:p-4 rounded-2xl w-full md:w-64 border border-slate-700/50 bg-slate-800/40">
          <div className="flex justify-between items-end mb-2">
            <div>
              <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Level {level}</p>
              <p className="text-base sm:text-lg font-bold text-white">{currentLevelXp} <span className="text-xs sm:text-sm font-normal text-slate-400">/ 100 XP</span></p>
            </div>
          </div>
