...
export function StatsCard({ title, value, icon: Icon, trend, trendUp }: StatsCardProps) {
  return (
    <div className="glass-card p-4 sm:p-6 border border-slate-800 bg-slate-800/30">
      ...
      <div className="flex items-baseline gap-2">
        ...
        {trend && (
          <span className={`text-xs sm:text-sm font-medium ${trendUp ? 'text-emerald-400' : 'text-slate-500'}`}>
            {trend}
          </span>
        )}
      </div>
